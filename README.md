...
API REST pública para gerenciamento financeiro pessoal, construída com **Spring Boot 3** e **Java 21**. Permite controlar usuários, contas bancárias, categorias, transações, além de expor um dashboard e relatórios financeiros — tudo protegido por autenticação JWT e isolado por usuário.

Projeto open source sob licença MIT — sinta-se livre para clonar, usar, modificar e contribuir.
...
**Transações** — criação e listagem paginada (padrão 20 itens, máximo 100, ordenada por `occurredAt` desc) de receitas e despesas. Cada transação está vinculada a uma conta e a uma categoria do próprio usuário, tem valor positivo, descrição opcional e data de ocorrência. O tipo da transação precisa ser compatível com o tipo da categoria (`INCOME`/`EXPENSE`).
...
| GET | `/transactions` | Lista as transações do usuário (paginado: `page`, `size`, `sort`) |
...
- Não é permitido registrar uma transação com tipo incompatível com o tipo da categoria (`CategoryTypeMismatchException`, `400`).
- E-mails duplicados e categorias duplicadas são rejeitados (`EmailAlreadyInUseException`, `CategoryAlreadyExistsException`).
- Ajustes de saldo em transações são feitos de forma atômica no banco, com bloqueios ordenados por UUID para evitar deadlocks; falhas de concorrência retornam `409` e podem ser reenviadas.
...
Suíte com 35 testes de integração (Testcontainers + MockMvc), cobrindo autenticação, CRUD com isolamento por usuário, atualização de saldo, validações, dashboard, relatórios e cenários de concorrência (race condition e deadlock). Usa o padrão Singleton Container para reaproveitar o mesmo container de banco entre as classes de teste (suíte completa em ~10s). Requer Docker rodando:
...
Distribuído sob a licença [MIT](LICENSE) — uso livre, inclusive comercial, com atribuição.
